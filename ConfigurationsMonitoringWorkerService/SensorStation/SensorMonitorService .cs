...
{
    internal class SensorMonitorService : BackgroundService
    {
        ...
        public SensorMonitorService(SensorFactory sensorFactory, IOptionsMonitor<SensorStationOptions> options, ILogger<SensorMonitorService> logger)
        {
        ...
        {
            _logger.LogInformation("""Current sensor station options:{Options}""", _options.CurrentValue);
        ...
        private void AlertSensorReadings(string sensor, double temp, ThresholdOptions thresholds)
        {
            ...
            {
                _logger.LogInformation("Normal '{Sensor}' reading: {Temp:F2}°F", sensor, temp);
            }
            else
            {
                // TODO: Send actual alert...

                _logger.LogCritical("""Wow! The '{Sensor}' reading is out of range: {Temp:F2}°F""", sensor, temp);
            }
        }

        private void OnOptionsChanged(SensorStationOptions latestOptions)
        {
            _logger.LogInformation("""Threshold's changed:{Options}""", latestOptions);
        }
